...
export default class OlimpTest extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataSource: {},
    ...
    getTest = () => {
        ...
        console.log("R", this.lesson);

        if (this.lesson.attempts && this.lesson.attempts > 0) {
            params = {};
            params.again = true;
        }

        Axios.get(`modules/tests/${this.lesson.id}/test/start`, { params })
            .then(res => {
                console.log("getTest", res);

                if (Array.isArray(res.data.data)) {
                    for (let i = 0; i < res.data.data.length; i++) {
                        for (let j = 0; j < res.data.data[i].answers.length; j++) {
                            res.data.data[i].answers[j].selected = false;
                        }
                    }
                }

                res.data.data.tests.forEach(item => {
                    this._ArrAnswers[item.id] = item.id;
                });

                console.log('this._ArrAnswers', this._ArrAnswers);
    ...
    SendTest = () => {
        ...
        Axios.post(`modules/tests/${this.state.dataSource.id}/test/finish`, null, {
            params: {
                _answers: Object.keys(this._ArrAnswers).length > 0 ? this._ArrAnswers : [],
            ...
            .then(res => {
                ...
                this.props.navigation.replace('TestResult', { lessonId: this.state.dataSource, title: this.lesson.title });
